...
| 计划驱动 | 每轮请求先在 `plans/` 生成或引用执行计划，再按计划推进文档、代码、测试 |
...
| [plans/](./plans/) | 执行计划 | 本轮执行依据 | 每轮请求开始；结构性变更必须生成或引用计划 |
...
□ 3. 在 `plans/` 生成或引用本轮执行计划；若计划缺失，先补计划
□ 4. 根据任务类型检索对应文档（见 §5 速查表）
□ 5. 若涉及变更，查阅 design/README.MD 联动关系表
□ 6. 按计划顺序推进文档、代码、测试，并记录偏离
□ 7. 任务完成后，若涉及结构性变更，更新 design/README.MD 修订记录
```
